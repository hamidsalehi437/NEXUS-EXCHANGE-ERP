...
> **وضعیت فعلی — فازهای ۰ تا ۴ تأیید شده‌اند؛ فاز ۵ (موتور معاوضهٔ ارز) آمادهٔ بازبینی است.**
...
> فاز ۴: موتور حسابداری — دفتر دوبل با `SUM(debit) = SUM(credit)` در سطح پایگاه‌داده، ثبت تغییرناپذیر، اصلاح با برگشت (reversal) و نه ویرایش، حسابداری چندارزی با نرخ نگه‌داشته‌شده در خط، ایدمپوتنسی ثبت و گزارش تراز آزمایشی. بازبینی مستقل دو حفرهٔ مرزی را یافت که هر دو بسته شد: جهت نامعتبر معاوضه و دور زدن محافظ موجودی از راه ثبت دستی. ۱۱۷۳ آزمون موفق. گزارش کامل: [`docs/phases/PHASE4_REPORT.md`](docs/phases/PHASE4_REPORT.md).
> فاز ۵: موتور معاوضه — چرخهٔ کامل خرید/فروش با اعتبارسنجی، عکس‌برداری نرخ از دفتر نرخ، کمیسیون و تسویه، سود/زیان واقعی، ثبت فقط از راه `AccountingService`، idempotency، قفل‌گذاری و آزمون‌های هم‌روندی روی PostgreSQL واقعی، ممیزی، شماره‌گذاری اسناد، مجوزها و ابطال/برگشت (بدون حذف). ۱۳۰۱ آزمون موفق. گزارش کامل: [`docs/phases/PHASE5_REPORT.md`](docs/phases/PHASE5_REPORT.md). فاز ۶ شروع نشده است.
>
> **Current status — Phases 0, 1, 2, 3 and 4 are APPROVED; Phase 5 (the exchange engine) is READY FOR REVIEW.** Phase 3 delivered the master data every later phase depends on. Phase 4 delivers the ledger those phases will post into: `AccountingService` as the only writer of journal entries, balance enforced in the database at COMMIT, immutable posted journals corrected by reversal, multi-currency posting that carries the rate it used, idempotent posting, branch and RBAC scoping, and the journal/trial-balance read models — with no schema change and no new migration, because the approved Phase 0 schema already covered the ledger. An independent Gate Review of the first submission found and closed two boundary holes (an invalid exchange *direction* that could reach posting, and the generic journal door bypassing the inventory-position guard), each pinned by its own regression suite. Phase 5 delivers the exchange engine on top of that ledger: the BUY/SELL lifecycle with validation before pricing, a deterministic rate snapshot taken from the rate book, gross/commission/net settlement, a realized result recomputed from the persisted entry, document numbers from the approved `next_document_number` series, idempotent writes (sequential, duplicate, concurrent, retry-after-failure), real-PostgreSQL concurrency protection over drawer positions, the cancel/reverse state machine with audit rows for every move, branch-scoped RBAC, and a deterministic receipt contract — again with no schema change and no new migration. **1301 tests pass** (126 of them new exchange tests). Phase 6 has **not** started. Full report: [`docs/phases/PHASE5_REPORT.md`](docs/phases/PHASE5_REPORT.md).
