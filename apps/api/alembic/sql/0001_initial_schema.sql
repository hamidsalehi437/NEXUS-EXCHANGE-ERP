...
-- Operational retention is not financial history: the maintenance worker's
-- idempotency sweep (PART 40, IDEMPOTENCY_RETENTION_DAYS) must be able to remove
-- records whose request can no longer be replayed. Exactly one table is granted
-- DELETE, it holds no money and no audit trail, and every revocation above still
-- holds. Nothing else in this schema may be deleted by the runtime role.
GRANT DELETE ON idempotency_keys TO nexus_app;
