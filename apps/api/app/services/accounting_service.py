...
class AccountingService:
    ...
    async def record_cash_movements(
        ...
        *,
        ...
        # The rows are collected and their ids read *after* the flush: ``id`` is generated by
        # the database (``UUIDPrimaryKeyMixin`` -> ``gen_random_uuid()``), so reading it
        # before the INSERT would return ``None`` and hand a caller an unusable movement id
        # (Phase 6 fix; the exchange path never read the return value, which is why the
        # defect survived Phase 5).
        inserted: list[CashMovement] = []
    ...
    async def post_cash_movement(
        ...
        *,
        ...
        ``resolve_rate`` asks the **ledger** to value the movement instead of accepting a
        rate from the caller: a receipt is valued at the house's own quote for the currency
        in the receiving direction, a disposal leaves at the account's carrying rate, and a
        disposal the position cannot cover is refused before anything is written. The cash
        module (Phase 6) always asks for this, so no client-supplied rate can reach the
        books; ``OPENING`` is the exception the model names (§6.1: the opening rate is
        captured in the request and audited), so a session opening passes its rate
        explicitly and leaves ``resolve_rate`` false.
        """
        ...
        async with self._ledger_transaction(session) as active:
            ...
            base = await _require_base_currency(active)
            if resolve_rate:
                # Decided under the account lock, from the ledger: two concurrent
                # disposals of one drawer cannot both price themselves off a stale read.
                rate = await self._resolve_cash_movement_rate(
                    active,
                    incoming=incoming,
            ...
            plan = _PostingPlan(
                ...
                # A drawer disposal is a delivery of money the branch must hold: the same
                # quantity guard the generic journal door runs (§13.7) applies here, so a
                # cash-out of a drawer that cannot cover it is refused as a domain error
                # (409, with the shortfall) rather than reaching the database's deferred
                # ``NEX01`` at COMMIT, which is the backstop and not the interface.
                guard_inventory=True,
            )
            return await self._post(
                active, plan=plan, actor=actor, idempotency_key=idempotency_key, endpoint=endpoint
    ...
    async def _resolve_cash_movement_rate(
        self,
        session: AsyncSession,
        *,
        incoming: bool,
        ...
        """The rate a cash movement is valued at (``ACCOUNTING_MODEL.md`` §6.4).

        The functional currency is 1 by definition. Anything else is valued by the ledger:
        what the business **receives** enters at the house's own quote for that currency in
        the receiving direction; what it **delivers** leaves at the account's carrying rate
        — functional value per unit actually held — which is also where a delivery the
        position cannot cover is refused (``INSUFFICIENT_BALANCE``, with the shortfall).

        Both reads happen after the account locks, so the number a posting uses cannot be
        changed by the posting next to it.
        """
        if currency_id == base.id:
            return Decimal(1)
        if incoming:
            currency = await _require_currency(
                CurrencyRepository(session), currency_id, "currency_id"
            )
            return await self._functional_rate(
                session,
                currency=currency,
                base=base,
                branch_id=branch_id,
        ...
        return await self._carrying_rate(
            session,
            account_id=cash_account_id,
            currency_id=currency_id,
            base_currency_id=base.id,
            branch_id=branch_id,
            disposing_quantity=quantity,
        )
...
# The chart's cash-inventory band, stated once for both the ledger (drawer resolution) and
# the cash module (position reads): the base currency is 1000 and every other currency takes
# the next code in the band.
CASH_ACCOUNT_CODE_PATTERN = r"^10[0-9]{2}$"


def in_cash_band(code: str) -> bool:
    """Whether an account code sits in the chart's cash-inventory band (§5)."""
    return _in_inventory_band(code)


def validate_money(value: Decimal, *, field: str) -> Decimal:
    """Public form of the ledger's money validator (``NUMERIC(30,10)``, exactly).

    A document service validates the numbers it is about to post with the *same* rules the
    ledger applies, so a value the database cannot store is refused as a bad request at the
    edge instead of surfacing as a server defect from the insert.
    """
    return _validated_money(value, field=field)


def require_positive_money(value: Decimal, *, field: str) -> Decimal:
    """Public form of the ledger's positive-amount validator (see :func:`validate_money`)."""
