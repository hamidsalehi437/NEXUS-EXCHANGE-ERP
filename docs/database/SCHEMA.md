...
| D-22 | **No structural change** for the rate snapshot (Phase 4 review, decision D-4-1) | Decision | The rate that priced a posting is *copied* into the append-only `journal_lines.exchange_rate` (from which `foreign_amount` is generated), and the quote it came from is recorded in the append-only, hash-chained `audit_logs.new_data.rate_snapshot`. No table references `exchange_rates` at all, and `journal_lines` carries no rate foreign key — so a quote may be replaced, superseded or re-priced without touching history, while the posted numbers cannot be re-priced by anyone. A FK to `exchange_rates` was considered and rejected: it would let a market-observation row veto financial history and would imply the referenced row is authoritative for an amount that is already frozen in the line. Reasoning and tests: `ACCOUNTING_MODEL.md` §14, `PHASE4_REPORT.md` §13 | A "reference" to a mutable quote would have been the only path by which a posted amount could change; denormalising the number removes that path entirely |
...
2. **Drift gate in CI.** The accepted gate is `apps/api/scripts/schema_gate.py`: `orm-db` compares the ORM metadata against a migrated database, and `db-db` compares the migrated database against the reference file applied to a second database. Both must report `MATCH` (31 tables, 341 columns, 72 indexes, 71 checks, 48 triggers, 23 routines, 5 views) — and CI runs both, then `tests/invariants/phase0_schema_invariants.sql` against a freshly migrated database. `alembic check` (autogenerate) is deliberately **not** part of the gate set: the reference file is applied verbatim as SQL, so autogenerate cannot see the SQLAlchemy-declared *names* of constraints and indexes created by literal DDL and reports them as pending on every database regardless of drift. Recorded as limitation L-10 in `../phases/PHASE4_REPORT.md`; a future phase may either attach the ORM to the DDL through `MetaData` naming or drop that step from this list.
